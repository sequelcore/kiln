...
| `sessionTurnBudget.*`, `managedAgents.*`, `managedAgents.intents[]` | CLI config; Runtime/Core budget and managed-agent owners | intent plus projected evidence | work limits, target/model choice, paid-usage posture, approval and worktree authority; high | global; next turn/session unknown |
...
| Available Models | secret-free discovery/configuration projection | guided preview only; no dispatch or mutation authority | discovery cache only | Runtime-owned managed economic observations are projected by Slice 7A |

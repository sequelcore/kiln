...
| [`runtime-budget-admission.ts`](../../packages/runtime/src/session/runtime-budget-admission.ts) | Session-turn provider budget admission; usage may be estimated or unavailable, and no reservation/settlement ledger exists. | [Issue #35](https://github.com/sequelcore/kiln/issues/35) remains the normal session-turn budget boundary. #19 must not silently replace it. |
...
| [#34](https://github.com/sequelcore/kiln/issues/34) | Managed-job cross-provider economic selection, account commitment, dispatch fencing, and settlement. #19 may use the same Runtime authority style but does not own monetary/provider-account state. |
| [#35](https://github.com/sequelcore/kiln/issues/35) | Normal `runtime-session-turn` budgeting. Provider-local daily ceilings and usage normalization remain its boundary until separately replaced. |

...
Still outstanding: managed-job V7 unification (a structurally separate headless/CLI job-queue subsystem, explicitly out of scope - see the Slice 6 plan), and the sanitization/replay-stability/malformed-version fail-closed projection tests. Roadmap Slice 5 (this doc's Slice 5, Cross-Path Account Authority Convergence, issue #39) is queued and unblocked now that dispatch proof exists, but not started.
Created: 2026-07-23

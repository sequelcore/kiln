...
- Route-unavailable managed tool outcomes now stay pre-invocation and
  structured across runtime metadata, operator event presentation, and GUI
  transcript rendering. `managed_agent.invoke` and `managed_agent.start`
...
- Direct-provider parent interruption now has explicit bridge regression
  coverage from `ProviderSession.run(...)` to runtime per-call config and from

...
5. Read `02-external-benchmark-validation.md` only when benchmark work becomes
   active.
...
3. Use `04-operator-surfaces-and-remote-gui.md` to sequence post-parity
   surface work: governed event timeline, IDE extension design, remote GUI
   hardening, and the Tauri decision gate.
4. Write and accept the config and registries surface ADR before broader config

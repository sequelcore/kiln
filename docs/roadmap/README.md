...
| --- | --- | --- | --- | --- |
| 1 | [02 - Public Release UI Debt](./02-public-release-ui-debt.md) | Ready | Add the contract-backed composer context-usage indicator. | It is the next public-release blocker and requires a bounded Gateway, runtime, GUI, and E2E slice. |
| 2 | [02 - Public Release UI Debt](./02-public-release-ui-debt.md) | Research | Define and verify cross-surface event-presentation density, then complete final live release validation. | The GUI symptom is known, but CLI/TUI parity rules require evidence before shared projection changes. |
...
| 4 | [06 - Cross-Harness Kiln Control Plane](./06-cross-harness-kiln-control-plane.md) | Research | Complete Slice 0 vocabulary/boundary evidence, then approve the smallest tool or managed-agent exposure slice needed to unblock Roadmap 04. | Cross-harness authority work is strategically important but broader and riskier than the release queue. |
| 5 | [04 - Verified Efficiency Control Plane](./04-verified-efficiency-control-plane.md) | Blocked | Resume Slice 3 telemetry, replay, and non-inferiority work. | Roadmap 06 must first define and verify the required cross-harness Kiln tool and agent exposure. |
| 6 | [05 - Skill Capability Plane](./05-skill-capability-plane.md) | Research | Define the provider-neutral skill evidence and admission contract before automatic operations or value promotion. | Inventory and repair evidence exist, but policy ownership and promotion evidence remain open. |
...
| [02 - Public Release UI Debt](./02-public-release-ui-debt.md) | Ready | Provider/model eligibility, Setup skill diagnostics, and several GUI foundations are complete. Authoritative context usage, event-density parity, and final live validation remain. | Add the contract-backed composer context-usage indicator. |

...
| --- | --- | --- | --- |
| 1 | [02 - Managed Invocation Routing](02-managed-invocation-routing.md) | Ready | Issue #34 internal Slice 5 dispatch was reviewed post-delivery and had two High findings (H1/H2) plus five Medium/Low findings, all now fixed. Internal Slice 6 has four bounded pieces delivered: Runtime canonical economic events proven to the Gateway-contracts frame boundary; a typed Gateway-contracts contract + shared cockpit projection rendered across CLI, TUI, and GUI (Native is out of active development and unwired); recovery-checkpoint reservation evidence (additive, backward-compatible); and the session-event jobId-to-invocationId join (a careful hoist inside the H1/H2-affected `prepareManagedInvocationRequest`, proven behavior-preserving by the full existing suite). Remaining Slice 6 work is managed-job V7 unification (explicitly deferred, separate subsystem) and replay-stability/malformed-version fail-closed projection tests. |

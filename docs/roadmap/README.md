...
| [05 - Cross-Harness Kiln Control Plane](./05-cross-harness-kiln-control-plane.md) | Ready | Slices 0-3 are complete. The governed Model Gateway dispatches all supported direct providers through exact same-provider account pools; Codex/OpenCode/Claude ingress projections and the fail-closed Claude read-only candidate are implemented. Slice 4 economics remain planned; Slice 5 awaits strict provider live proof. | Define the Slice 4 quota and subscription evidence contract, then obtain strict Claude provider proof before admitting any Claude model. Keep setup, dogfood, and benchmark slices separately gated. |

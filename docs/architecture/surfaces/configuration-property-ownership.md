...
| `P` | CLI `KilnProjectConfig` | CLI project composition | `readKilnYaml` | `writeKilnYaml`, proposal/apply | run, status, GUI/TUI, Tools MCP | `.kiln/kiln.yaml`; project |
...
| --- | --- | --- |
| Global config mutation | CLI `mutateGlobalConfig`; validation, revision CAS, lock, same-directory temporary file, atomic replace, invalid backup | retain as the global application-port primitive in Slice 4 |
...
| Project init/config set | CLI `writeKilnYaml`; whole-object serialization, no shared atomic lifecycle | replace with AST mutation and typed result in Slice 4 |
| Project proposal/approval/apply | CLI application services; hash/path fences, sequential writes, no atomic multi-file commit, reconciliation failure can be overstated | converge on one Slice 4 application port and honest terminal result |

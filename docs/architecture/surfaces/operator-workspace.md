...
Browser session observation carries projected `gatewayTargetId` when browser
session state includes target identity. The runtime GUI gateway forwards that
identity into browser-session evidence so inspection can be correlated back to

...
- Closed and retired `02-native-operator-surface-foundation.md` on
...
- High-volume artifact links can now store an internal resource payload instead
  ...
  packet.
- Promoted the completed shared tooling intelligence and context resource plane
...
- Added the runtime `operator_set_theme` tool for attached CLI/GUI/TUI
  sessions. Executable providers can request operator theme changes through the
  same governed tool loop as other builtin tools.
...
- Added `OpenCodeRateLimitError` and `OpenCodeQuotaError` with `provider`
  context and `retryable` flags, shaped for the provider credential pool
  roadmap.
...
- Added the provider credential pool roadmap as the forward plan

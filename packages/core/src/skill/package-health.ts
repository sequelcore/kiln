...
import { loadSkillMdIndex } from "./md-parser.js";
...
export interface SkillPackageHealth {
  ...
  readonly brokenResources: readonly { readonly source: string; readonly target: string; readonly reason: "missing" | "outside-package" }[];
...
export function inspectSkillPackage(root: string, options: SkillPackageHealthOptions = {}): SkillPackageHealth {
  ...
  const walk = (current: string, depth: number): void => {
    ...
    if (depth > limits.maxDepth) {
      diagnostics.push({ code: "package-too-deep", message: `Skill package exceeds depth ${limits.maxDepth}.`, path: portable(current) });
      return;
    }
    let entries;
    try { entries = readdirSync(current, { withFileTypes: true }); } catch {
      diagnostics.push({ code: "resource-unreadable", message: "Skill package directory is unreadable.", path: portable(relative(physicalRoot, current)) });
      return;
    }
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      if (stat.isSymbolicLink()) {
        diagnostics.push({ code: "package-symlink", message: "Symbolic links are not admitted inside skill packages.", path: rel });
        ...
        if (stat.size > limits.maxFileBytes) {
          diagnostics.push({ code: "oversized-file", message: `Skill package file exceeds ${limits.maxFileBytes} bytes.`, path: rel });
          continue;
        }
        if (packageBytes + stat.size > limits.maxPackageBytes) {
          diagnostics.push({ code: "oversized-package", message: `Skill package exceeds ${limits.maxPackageBytes} bytes.` });
          return;
        }
        try {
  ...
  let index;
  try { index = loadSkillMdIndex(skillPath); } catch (error) {
    diagnostics.push({ code: "invalid-skill", message: error instanceof Error ? error.message : "Invalid SKILL.md.", path: "SKILL.md" });
  }
  if (index && (index.name.length > 64 || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(index.name) || index.description.length > 1024)) {
    diagnostics.push({ code: "portable-spec-invalid", message: "Skill name or description violates the portable Agent Skills identity constraints.", path: "SKILL.md" });
  }
  const brokenResources: { source: string; target: string; reason: "missing" | "outside-package" }[] = [];
  for (const file of files) for (const target of markdownReferences(file)) {
  ...
  function blocked(code: string, message: string): SkillPackageHealth {
    return { status: "blocked", fileCount: 0, packageBytes: 0, brokenResources: [], riskSignals: [], diagnostics: [{ code, message }] };
  }
}
...
function collectRiskSignals(files: readonly { path: string; content: Buffer }[]) {
  ...
  for (const file of files) {
    ...
    if (/\b(?:authorization|api[_-]?key|access[_-]?token|bearer\s+)[\s:=]/i.test(text)) add("credential-pattern", file.path);
    if (/(?:^|[\s'"`(])\.\.\//m.test(text) || /(?:^|[\s'"`(])[A-Za-z]:[\\/]/m.test(text)) add("outside-filesystem-access", file.path);
  }
  return results;
}

...
describe("skill package health", () => {
  ...
  it("fails closed for broken, escaping, and oversized resources", () => {
    ...
    writeFileSync(join(root, "SKILL.md"), skill("Use [missing](references/missing.md) and [outside](../secret.md)."), "utf8");

    const health = inspectSkillPackage(root, { maxFileBytes: 512, maxPackageBytes: 2_048 });
    expect(health.status).toBe("blocked");
    expect(health.brokenResources).toEqual(expect.arrayContaining([
  ...
  it("blocks Windows drive and UNC Markdown resources on every host platform", () => {
    const root = packageRoot();
    writeFileSync(join(root, "SKILL.md"), skill("Read [drive](C:\\secrets\\guide.md) and [share](\\\\server\\share\\guide.md)."), "utf8");

    expect(inspectSkillPackage(root).brokenResources).toEqual(expect.arrayContaining([
  ...
  it("classifies executable, network, secret, and broad filesystem risk signals", () => {
    ...
    writeFileSync(join(root, "scripts", "run.sh"), "curl https://example.invalid -H 'Authorization: Bearer token'\ncat ../../private\n", "utf8");
    writeFileSync(join(root, "SKILL.md"), skill("Run `scripts/run.sh`."), "utf8");

    expect(inspectSkillPackage(root).riskSignals.map((signal) => signal.kind)).toEqual(expect.arrayContaining([
      "code-execution", "network-access", "credential-pattern", "outside-filesystem-access",
    ]));
  });

  it("blocks packages that violate portable Agent Skills identity constraints", () => {
    const root = packageRoot();

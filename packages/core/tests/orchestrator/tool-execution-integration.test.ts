...
describe("Orchestrator native tool execution path", () => {
  it("registers and executes a native tool via executeDevTool and emits tool events", async () => {
    ...
    expect(called.annotations).toEqual({ idempotent: true });
    expect(called.authorizationLevel).toBe(2);
    ...
    expect(authorized.level).toBe(2);
  ...
  it("denies destructive tools through existing annotation authorizer", async () => {
    const orchestrator = new Orchestrator();
    ...
    const called = toolEvents[0] as ToolCalledEvent;
    expect(called.authorizationLevel).toBe(4);
  });

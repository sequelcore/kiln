...
describe("MemoryGraphResourceProvider", () => {
  ...
  it("lists memory graph resources and templates through the canonical registry and model-callable tools", async () => {
    ...
    expect(registry.listTemplates().map((template) => template.uriTemplate)).toEqual(expect.arrayContaining([
      ...
      "kiln://memory/admissions{?sessionId,recordId,limit}",
    ]));
    ...
    expect(JSON.parse(templateResult.output).resourceTemplates.map((template: { uriTemplate: string }) => template.uriTemplate)).toContain(
      "kiln://memory/graph{?scope,scopeKind,scopeId,layer,query,depth,limit}",
    );
  });
  ...
  it("reads node detail, provenance, neighbors, relation, and admission resources", async () => {
    ...
    expect(node.relations.map((candidate: { id: string }) => candidate.id)).toEqual([relation.id]);
    expect(provenance.provenance).toEqual(root.provenance);
    expect(provenance.admissions.map((admission: { id: string }) => admission.id)).toEqual(["admission-1"]);
    expect(neighbors.snapshot.nodes.map((graphNode: { recordId: string }) => graphNode.recordId)).toEqual([root.id, child.id]);
    expect(relationPayload.relation).toMatchObject(relation);
    expect(admissions.admissions.map((admission: { id: string }) => admission.id)).toEqual(["admission-1"]);
  });

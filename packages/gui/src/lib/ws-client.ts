...
const GuiInteractiveUseSnapshotSchema = z.object({
  ...
  toolCallScopeId: z.string().optional(),
...
const GuiBrowserSessionStateSchema = z.object({
  ...
  title: z.string().optional(),
  visibleText: z.string().optional(),
  ownership: z.enum(["agent", "released"]),
  ...
  latestCapture: z.object({
    ...
    transport: z.enum(["snapshot-polling", "cdp-screencast", "webrtc", "hosted-url"]).optional(),
  }).optional(),
  actionSummary: z.string().optional(),
  error: z.string().optional(),
});
...
const GuiSessionEventSchema = z.object({
  ...
  source: z.object({
    ...
    surface: z.enum(["cli", "tui", "gui", "ide", "sdk", "widget", "gateway", "runtime"]),
...
const GuiInboundFrameSchema = z.union([
  ...
  z.object({
    ...
    transport: z.enum(["snapshot-polling", "cdp-screencast", "webrtc", "hosted-url"]),
  ...
  z.object({ type: z.literal("provider_catalog_state"), status: z.enum(["pending", "refreshing"]) }),
  z.object({
    type: z.literal("provider_catalog_state"),

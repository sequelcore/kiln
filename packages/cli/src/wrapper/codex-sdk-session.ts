...
export class CodexSdkSession implements IKilnSession {
  ...
  async *run(options: SessionRunOptions): AsyncIterable<ExecutionSessionEvent> {
    ...
    const prompt = withConstraints(
      appendExecutionIdentity(options.system ? `${options.prompt}\n\n--- Kiln Prepared System Context ---\n${options.system}` : options.prompt, resolveExecutionIdentity({ configuredProvider: "codex", configuredModel: model, configuredCanonicalModel: model, configuredBillingMode: "unknown" })),
    ...
    try {
      const thread = resumeId ? this.port.resumeThread(resumeId, threadOptions) : this.port.startThread(threadOptions);
...
function officialPort(env?: Record<string, string>): CodexSdkPort { const merged = { ...process.env, ...env }; delete merged.CODEX_MODEL; return new Codex({ codexPathOverride: resolveNativeCliExecutable({ command: "codex", fallbackPaths: [`${process.env.HOME ?? process.env.USERPROFILE ?? ""}\\.codex\\.sandbox-bin\\codex.exe`] }), env: merged as Record<string, string> }); }

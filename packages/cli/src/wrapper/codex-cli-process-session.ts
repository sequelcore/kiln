...
/**
 * Exact CLI transport for public switches not represented by SDK 0.147.0.
 * It is selected deliberately by CodexSession, never as a catch-all fallback.
 */
export class CodexCliProcessSession implements IKilnSession {
  ...
  async *run(options: SessionRunOptions): AsyncIterable<ExecutionSessionEvent> {
    ...
    const permissionRequest = this.config.runtimePermissionObservationSink
      ? await this.config.runtimePermissionObservationSink.recordRequested(deriveCodexRuntimePermissionRequest({
      sessionId: this.sessionId,
      approvalMode: this.config.approvalMode ?? "on-request",
      sandboxMode: this.config.sandboxMode ?? "read-only",
    ...
    const onStderrData = (chunk: Buffer): void => {
      ...
      if (Buffer.byteLength(text) > remaining) stderrTruncated = true;
    };
    ...
    try {
      ...
      switch (line.type) {
        case "thread.started":
          ...
          if (permissionRequest) await this.config.runtimePermissionObservationSink!.recordObserved(permissionRequest, { observedAt: new Date(), proof: "inferred" });
          break;

...
export function buildWorkGovernanceContext(config: KilnWorkGovernanceConfig | undefined): string | undefined {
  ...
  const lines = [
    ...
    "",
    "Interpretation:",
    "- Direct execution is the baseline. Orchestrate only when a configured delegation trigger matches or coordination has a concrete expected-value source such as independent parallel work, specialization, context isolation, independent evidence, or an authority boundary.",
    ...
    "- Decompose non-trivial executable work where concerns vary independently. Delegate only when coordination is expected to improve the result, then verify and close with evidence.",
    ...
    "- If work_item.execution.start pauses for managed delegation, call managed_agent.invoke with the exact managedInvocationRequest object it returned; do not add agentProfile when it is absent, and do not replace a route-owned request with a guessed access level.",
    ...
    "- Model self-confidence and independent LLM review are not verification; prefer executable checks, browser QA, formal proof, externally grounded evidence, and residual-risk reporting.",

...
export async function createCurrentExecutionRoute(input: {
  ...
  readonly commit?: (input: {
    readonly draft: import("./execution-route-draft.js").CompleteExecutionRouteDraft;
  ...
  try {
    ...
    const commit = await (input.commit ?? (async (commitInput) => {
      const committed = await createExecutionRoute(commitInput);
      return {
    ...
    return {
      ...
      revision: commit.revision,
      proposal: revalidated.proposal,
      executionRouteCatalog: projectExecutionRouteCatalog(refreshed.executionCatalog, refreshed.revision),
      availableModels: refreshed.catalog,
    };
  } catch (error) {
...
function projectExecutionRouteCatalog(catalog: ExecutionCatalogInput, revision: string): ExecutionRouteCatalog {
  return {
    ...
    routes: catalog.routes.map((route) => {
      const base = {
        ...
        reasonCodes: ["route-evidence-pending" as const],
        repairActions: ["refresh-route-catalog" as const],
      };
      if (route.accountSelection.mode === "automatic") {
        const automaticSelection = route.accountSelection;
        return {
          ...base,
          accountSelection: {
            mode: "automatic" as const,
            eligibleAccountCount: catalog.accountPolicies.find((policy) => policy.id === automaticSelection.accountPolicyId)?.accountIds.length ?? 0,
            allowOperatorOverride: true as const,
          },
        };
      }
      return {
        ...
        accountSelection: {
          mode: "exact" as const,
          eligibleAccountCount: 1 as const,
          allowOperatorOverride: false as const,
        },
      };
    }),
  };

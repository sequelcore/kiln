 ...
 * validated, external directory and returns facts that a caller may later
...
export function loadPrivateFormalScreeningPackage(
  ...
  const publishRoots = [repositoryRoot, ...(options.publishSurfaceRoots ?? [])]
  ...
  if (publishRoots.some((surface) => pathsOverlap(surface, rootPath))) {
    throw new Error("Private formal screening package root must remain outside the repository and publish surfaces.");
  }

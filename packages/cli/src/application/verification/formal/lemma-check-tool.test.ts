...
describe("lemma_check", () => {
  it("runs the same qualification as a sealed host observation without model input", async () => {
    const fixture = createFixture();
    const output = await qualifyLemmaCandidateSealed(fixture.workspacePath, {
      ...fixture.options,
      runner: async () => successfulSubprocess(fixture.qualificationResult),
    });
    const sourceDigest = digest(readFileSync(join(fixture.workspacePath, "src", "solution.ts")));

    expect(isPassedLemmaQualification(output, sourceDigest)).toBe(true);
    expect(hasCleanLemmaTrustPolicy(output)).toBe(true);
    expect(isLemmaQualificationInfrastructureFailure(output)).toBe(false);
    expect(output.semanticEquivalence).toBe("unresolved");
    expect(output.benchmarkReady).toBe(false);
  });

  it("distinguishes a model-damaged candidate from sealed qualification infrastructure failure", async () => {
    const candidateFixture = createFixture();
    rmSync(join(candidateFixture.workspacePath, "src", "solution.ts"));
    const candidateFailure = await qualifyLemmaCandidateSealed(
      candidateFixture.workspacePath,
      candidateFixture.options,
    );
    expect(candidateFailure).toMatchObject({ stage: "input", diagnosticCodes: ["candidate-invalid"] });
    expect(isLemmaQualificationInfrastructureFailure(candidateFailure)).toBe(false);

    const infrastructureFixture = createFixture();
    const infrastructureFailure = await qualifyLemmaCandidateSealed("relative-workspace", infrastructureFixture.options);
    expect(infrastructureFailure).toMatchObject({ stage: "input", diagnosticCodes: ["invalid-input"] });
    expect(isLemmaQualificationInfrastructureFailure(infrastructureFailure)).toBe(true);
  });

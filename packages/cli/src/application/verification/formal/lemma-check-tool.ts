...
const DEFAULT_QUALIFICATION_SCRIPT_PATH = fileURLToPath(
  new URL("../../../../../../scripts/verification/lemma-script/qualification.ts", import.meta.url),
);
...
export class LemmaCheckTool implements DevTool {
  ...
  private async screen(input: ToolInput): Promise<LemmaCheckOutput> {
    ...
    return qualifyLemmaCandidateSealed(this.workspacePath, this.options);
  }
}

/** Run the fixed qualification as a host-only outcome observation. */
export async function qualifyLemmaCandidateSealed(
  workspacePath: string,
  options: LemmaCheckToolOptions,
): Promise<LemmaCheckOutput> {
    const configError = validateOptions(workspacePath, options);
    if (configError !== undefined) return makeFailure("input", [configError]);
    const expectedVersions = configuredVersions(options.toolchain);

    const paths = await resolveHostPaths(workspacePath);
    if (typeof paths === "string") return makeFailure("input", [paths], expectedVersions);
    const toolchainError = await validateToolchain(options.toolchain);
    if (toolchainError !== undefined) return makeFailure("input", [toolchainError], expectedVersions);
    const toolchainBefore = await readToolchainDigests(options.toolchain);
    ...
    const request = createQualificationRequest(paths, options);
    const runner = options.runner ?? runBunQualificationProcess;
    ...
    const toolchainAfter = await readToolchainDigests(options.toolchain);
    ...
    const parsed = parseQualificationOutput(processResult.stdout, options);
...
export function isPassedLemmaQualification(
  observation: unknown,
  ...
  return observation.kind === "pipeline_passed"
    ...
    && validDigest(digests.generated)
    && validDigest(digests.lemmaScriptExecutable)
    && validDigest(digests.dafnyExecutable)
    && validDigest(digests.dependencyBinding)
...
export function hasCleanLemmaTrustPolicy(observation: unknown): boolean {
...
const INFRASTRUCTURE_FAILURE_STAGES = new Set<LemmaCheckOutputStage>([
  "versions",
  "tool_integrity",
  "cleanup",
  "infrastructure",
...
export function isLemmaQualificationInfrastructureFailure(observation: LemmaCheckOutput): boolean {
  if (observation.stage === "input") {
    return !observation.diagnosticCodes.every((code) => code === "candidate-invalid");
  }
  return observation.status !== "passed" && INFRASTRUCTURE_FAILURE_STAGES.has(observation.stage);
}
...
async function resolveHostPaths(workspacePath: string): Promise<HostPaths | string> {
  ...
  try {
    ...
    if (!isContained(canonicalWorkspace, canonicalCandidate)) return "candidate-invalid";
    return {

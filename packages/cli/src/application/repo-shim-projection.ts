...
type RepoShimKind = "agents-md" | "claude-md";
...
export async function writeRepoShimProjections(
  ...
  try {
    ...
    const projectRootId = hashText(projectPath.toLowerCase()).slice(0, 16);
...
function renderRepoShimBody(input: {
  ...
  const { target, agents, instructionProfiles, kilnYaml, repoContext } = input;
  ...
  const lines: string[] = [
    target.kind === "claude-md" ? "# Claude Project Guidance" : "# Agents",
    "",

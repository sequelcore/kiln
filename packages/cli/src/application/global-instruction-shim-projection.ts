...
import { readGlobalConfigSnapshotAtPath } from "../config/config-merger.js";
...
export interface GlobalInstructionShimProjectionOptions {
  ...
  /** Established project identity; never an authority source for global projections. */
...
async function loadProjectionContext(
  ...
  const globalKilnHome = options.userHome ? join(home, ".kiln") : resolveKilnHomePath();
  const globalConfig = readGlobalConfigSnapshotAtPath(join(globalKilnHome, "config.yaml")).config;
  const allProfiles = loadGlobalInstructionProfiles(options.userHome, { kilnHome: globalKilnHome });
  const activeProfiles = globalConfig?.activeInstructionProfiles ?? [];
...
function renderGlobalInstructionShimBody(
  ...
  return [
    ...
    "",
    ...renderAuthoritySection(target),
    ...renderInstructionProfilesSection(context.profiles),
  ].join("\n");
}

function renderAuthoritySection(target: GlobalInstructionShimTarget): readonly string[] {
  return [
    "## Authority",
    "",
...
function harnessProjectGuidance(harness: GlobalInstructionShimHarness): string {
  switch (harness) {
    case "codex":
      return "Codex layers project-owned `AGENTS.md` files after this global guidance.";
    case "claude":
      return "Claude Code layers project-owned `CLAUDE.md` files after this user guidance; a project may import `@AGENTS.md`.";
    case "opencode":
      return "OpenCode layers the project-owned `AGENTS.md` after this global guidance.";
  }
}

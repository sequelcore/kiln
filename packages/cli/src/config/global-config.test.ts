...
describe("global-config", () => {
  ...
  it("readGlobalConfig() rejects invalid execution catalog references and account selection", () => {
    ...
    expect(() => readGlobalConfig()).toThrow("executionCatalog.routes[0].dataClassification is invalid");

    readFileSyncMock.mockReturnValue(valid.replace("expiresAt: 2027-01-01T00:00:00Z", "expiresAt: 2027-01-01T00:00:00Z, rawPolicy: secret"));
    expect(() => readGlobalConfig()).toThrow("Unknown executionCatalog.routes[0].dataPolicyEvidence field: rawPolicy");
  });

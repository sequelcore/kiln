...
export async function createCli(config: KilnAppConfig): Promise<void> {
  ...
  const COMMANDS: Record<string, string> = {
    ...
    "managed-economic": "Reconcile economic reservations using reviewed no-dispatch evidence",
  ...
  if (command === "managed-economic") {
    const { managedEconomicCommand } = await import("./commands/managed-economic.js");
    await managedEconomicCommand(args.slice(1));
    return;
  }

...
describe("runtime configuration revision", () => {
  ...
  it("captures the latest path-scoped settlement lineage when a revision reappears after rollback", () => {
    ...
    const projectPath = join(root, ".kiln", "kiln.yaml");
    const globalBytes = `version: "4"\ntargetCatalog:\n  evidenceRevision: sha256:${"e".repeat(64)}\n`;
    const projectBytes = "version: \"1\"\nprojectName: fixture\n";
    writeFileSync(globalPath, globalBytes, "utf8");
    writeFileSync(projectPath, projectBytes, "utf8");
  ...
  it("does not report lineage when current bytes resurrect an old revision without a newer rollback settlement", () => {
    ...
    writeFileSync(globalPath, globalBytes, "utf8");
    writeFileSync(projectPath, projectBytesA, "utf8");
    ...
    store.settle(lineageSettlement({
  ...
  it("retries capture when settlement lineage changes while canonical bytes remain stable", () => {
    ...
    const readLineage = vi.spyOn(ConfigMutationStore.prototype, "readLatestSettlementForPath").mockImplementation(function (
      this: ConfigMutationStore,
      path,
      revision,
    ) {
      reads += 1;
  ...
  it("admits shared global lineage across projects and selects a same-hash rollback", () => {
    ...
    mkdirSync(join(projectB, ".kiln"), { recursive: true });
    mkdirSync(join(home, "kiln"), { recursive: true });
    const globalPath = join(home, "kiln", "config.yaml");
    const globalBytes = "version: \"4\"\n";
    writeFileSync(globalPath, globalBytes, "utf8");
    ...
    storeA.settle(lineageSettlement({ proposalId: "cfg-global-a", path: globalPath, scope: "global", committedRevision: revision, settledAt: "2026-08-22T00:00:00.000Z", generation: `sha256:${"a".repeat(64)}` }));
    storeA.settle(lineageSettlement({ proposalId: "cfg-global-rollback-a", path: globalPath, scope: "global", committedRevision: revision, settledAt: "2026-08-22T00:00:01.000Z", generation: `sha256:${"b".repeat(64)}`, operation: "mutation.rollback" }));

    const admittedByB = readRuntimeConfigurationRevision(projectB, { globalConfigPath: globalPath, mutationStoreRoot: mutationRoot });

    expect(admittedByB.activationLineage?.[0]).toMatchObject({ proposalId: "cfg-global-rollback-a", scope: "global", path: "config.yaml" });
  });
});

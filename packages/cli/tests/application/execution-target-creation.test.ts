...
import { createExecutionTarget } from "../../src/application/execution-target-creation.js";
...
describe("createExecutionTarget", () => {
  ...
  it("commits target intent through the authority with exact config and evidence revisions", async () => {
    ...
    const result = await createExecutionTarget(input);
  ...
  it("reports a committed revision when reconciliation fails after commit", async () => {
    ...
    const result = await createExecutionTarget(creationInput());
  ...
  it("reports an authority rejection without claiming the target was created", async () => {
    ...
    await expect(createExecutionTarget(creationInput())).rejects.toThrow("revision drift");
  });

  it("does not mint durable approval without an explicit operator decision", async () => {
    await expect(createExecutionTarget({ ...creationInput(), operatorApproved: false }))
  ...
  it("validates the complete intent/evidence pair before publishing either authority", async () => {
    ...
    await expect(createExecutionTarget({
      ...input,
      draft: {
        ...input.draft,
        evidence: {
...
function completeDraft() {
  const economics = targetEconomics();
  return {

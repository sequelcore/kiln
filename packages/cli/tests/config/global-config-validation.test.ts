...
describe("validateGlobalConfig root fields", () => {
  it.each([
    { version: 1, harnesses: { codex: { keepImplicit: [] } } },
    { version: 2, harnesses: { codex: { expectedFingerprint: `sha256:${"a".repeat(64)}`, keepImplicit: [] } } },
    { version: 2, harnesses: { codex: { keepImplicit: [{ sourceId: " " }] } } },
    { version: 2, harnesses: { claude: { keepImplicit: [] } } },
  ])("rejects obsolete evidence fields, policy versions, and unsupported selections", (externalCatalog) => {
    expect(() => validateGlobalConfig({ ...baseConfig(), skills: { externalCatalog } })).toThrow(
      /skills.externalCatalog/u,
    );
  });
  ...
  it("accepts a versioned reviewed Codex external catalog policy and rejects ambiguous decisions", () => {
    const decision = { sourceId: "plugin:docs:pdf:." };
    expect(() =>
      validateGlobalConfig({
        ...baseConfig(),
        skills: { externalCatalog: { version: 2, harnesses: { codex: { keepImplicit: [decision] } } } },
      }),
    ).not.toThrow();
    expect(() =>
      validateGlobalConfig({
        ...baseConfig(),
        skills: { externalCatalog: { version: 2, harnesses: { codex: { keepImplicit: [decision, decision] } } } },
      }),
    ...
    expect(() =>
      validateGlobalConfig({
        ...
        skills: {
          externalCatalog: {
            version: 2,
            harnesses: { codex: { keepImplicit: [{ ...decision, packageDigest: "sha256:bad" }] } },
          },
        },
      }),
  ...
  it("rejects retired root routing and models fields", () => {
    expect(() => validateGlobalConfig({ ...baseConfig(), routing: {} })).toThrow(
      /Unknown global config field: routing/u,
    );

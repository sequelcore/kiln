...
describe("global control-plane MCP projection", () => {
  ...
  it("migrates only exact project-local Kiln bridge entries after global installation", async () => {
    const root = temporaryRoot();
    const userHome = join(root, "home");
    const projectPath = join(root, "project");
    const launch = syntheticLaunch(root);
    mkdirSync(join(projectPath, ".codex"), { recursive: true });
    const localCodexPath = join(projectPath, ".codex", "config.toml");
    writeFileSync(localCodexPath, [
      '[mcp_servers.kiln-control-plane]',
      'command = "kiln"',
      `args = ["native-harness", "control-plane-mcp", "--harness", "codex", "--project-root", ${JSON.stringify(projectPath)}]`,
      '[mcp_servers.keep]',
      'command = "keep"',
      "",
    ].join("\n"));
    const localCodexBefore = parseToml(readFileSync(localCodexPath, "utf8")) as Record<string, unknown>;
    writeNativeProjectionInstallState(join(projectPath, ".kiln"), upsertNativeProjectionTargetState(
      emptyNativeProjectionInstallState(),
      createNativeProjectionSnapshot({
        targetId: "mcp:codex",
        filePath: localCodexPath,
        document: localCodexBefore,
        managedFields: ["/mcp_servers/kiln-control-plane", "/mcp_servers/keep"],
        updatedAt: "2026-08-08T00:00:00.000Z",
      }),
    ));
    const unmanagedClaudeLegacy = {
      type: "stdio", command: "kiln",
      args: ["native-harness", "control-plane-mcp", "--harness", "claude", "--project-root", projectPath],
    };
    writeFileSync(join(projectPath, ".mcp.json"), JSON.stringify({ mcpServers: {
      "kiln-control-plane": unmanagedClaudeLegacy,
      keep: { command: "keep" },
    } }));

    const result = await syncGlobalControlPlaneMcpProjections({
      operation: "install", userHome, projectPath, launch, harnesses: ["codex", "claude"],
    });

    expect(result.targets).toEqual(expect.arrayContaining([
      expect.objectContaining({ harness: "codex", migratedLegacy: true }),
      expect.objectContaining({ harness: "claude", migratedLegacy: false }),
    ]));
    expect(parseToml(readFileSync(resolveGlobalControlPlaneMcpProjectionPaths(userHome).codex, "utf8"))).toMatchObject({
      mcp_servers: { "kiln-control-plane": {
        command: launch.executable,
        args: [launch.entrypoint, "native-harness", "control-plane-mcp", "--harness", "codex"],
      } },
    });
    const localCodex = parseToml(readFileSync(join(projectPath, ".codex", "config.toml"), "utf8")) as { mcp_servers?: Record<string, unknown> };
    expect(localCodex.mcp_servers?.["kiln-control-plane"]).toBeUndefined();
    expect(localCodex.mcp_servers?.keep).toEqual({ command: "keep" });
    const legacyState = JSON.parse(readFileSync(join(projectPath, ".kiln", "install-state.json"), "utf8"));
    expect(legacyState.targets["mcp:codex"].managedFields).toEqual(["/mcp_servers/keep"]);
    expect(JSON.parse(readFileSync(join(projectPath, ".mcp.json"), "utf8")).mcpServers["kiln-control-plane"]).toEqual(unmanagedClaudeLegacy);
  });

  it("preflights an owned malformed legacy file before changing the global projection", async () => {
    const root = temporaryRoot();
    const userHome = join(root, "home");
    const projectPath = join(root, "project");
    mkdirSync(join(projectPath, ".codex"), { recursive: true });
    const localPath = join(projectPath, ".codex", "config.toml");
    writeFileSync(localPath, "not = [valid", "utf8");
    writeNativeProjectionInstallState(join(projectPath, ".kiln"), {
      ...
      targets: {
        "mcp:codex": {
          ...
          managedFields: ["/mcp_servers/kiln-control-plane"],
          managedFieldHashes: { "/mcp_servers/kiln-control-plane": "synthetic" },
          updatedAt: "2026-08-08T00:00:00.000Z",
        },
      },
    });

    const result = await syncGlobalControlPlaneMcpProjections({ operation: "install", userHome, projectPath, harnesses: ["codex"] });

    expect(result.targets).toEqual([expect.objectContaining({ status: "blocked-malformed", changed: false })]);
    expect(existsSync(resolveGlobalControlPlaneMcpProjectionPaths(userHome).codex)).toBe(false);
  });

...
import type { ModelCatalog, GuiProviderDiscoveryResult } from "@kilnai/gateway-contracts";
...
vi.mock("../src/ui.js", () => ({
  ...
  destroyExecutionTargetPicker: () => undefined,
}));
  ...
  models: [
    {
      ...
      provenance: [],
      targets: [{ targetId: "claude-default", label: "Claude", access: "subscription", eligibleAccountCount: 1, accountOverrideIds: [], availability: "available", reasonCodes: ["configured"], repairActions: [], cost: { kind: "subscription" } }],
    },
    {
      providerId: "openai",
      ...
      availability: "available",
      provenance: [],
      targets: [{ targetId: "openai-gpt-5", label: "OpenAI GPT-5", access: "api", eligibleAccountCount: 2, accountOverrideIds: ["work", "personal"], availability: "available", reasonCodes: ["configured"], repairActions: [], cost: { kind: "metered", currency: "USD" } }],
    },
    {
      ...
      provenance: [],
      targets: [{ targetId: "codex-oauth", label: "Codex OAuth", access: "subscription", eligibleAccountCount: 1, accountOverrideIds: [], availability: "unavailable", reasonCodes: ["missing-credentials"], repairActions: ["authenticate-provider"], cost: { kind: "subscription" } }],
    },
  ],
};
...
async function openExecutionTargetPickerAndChooseOpenAi(): Promise<void> {
  emitText("/target");
  emitKey("\r", "return");
  await flushUi();
  expect(harness.state?.executionTargetPickerOpen).toBe(true);
  emitKey("\x1b[B", "down");
  await flushUi();
  emitKey("\r", "return");
  await flushUi();
...
describe("TUI execution-target picker", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.clearAllMocks();
    ...
    harness.state = null;
    harness.ui = null;
    harness.switchExecutionTarget.mockReset();
  ...
  it("does not update derived evidence until execution-target selection resolves successfully", async () => {
    let resolveSwitch: ((provider: string) => void) | undefined;
    harness.switchExecutionTarget.mockImplementation(
    ...
    const createSession = vi.fn(async () => ({
      ...
      dispose: vi.fn(),
      modelCatalog: MODEL_CATALOG,
      switchExecutionTarget: harness.switchExecutionTarget,
    }));
    ...
    try {
      await waitForTuiReady();
      await openExecutionTargetPickerAndChooseOpenAi();

      expect(harness.switchExecutionTarget).toHaveBeenCalledWith("openai-gpt-5");
  ...
  it("preserves derived execution evidence when route selection rejects with a connection failure", async () => {
    harness.switchExecutionTarget.mockImplementation(async () => {
  ...
  it("keeps configured route evidence separate from provider model discovery", async () => {
    harness.switchExecutionTarget.mockResolvedValue("openai-gpt-5");
    ...
    try {
      ...
      expect(harness.state?.currentProvider).toBe("openai");
      expect(harness.state?.currentModel).toBe("gpt-5.4");
      expect(harness.state?.executionTargetPickerOpen).toBe(false);
    } finally {
      harness.renderer?.destroy();
      void startPromise.catch(() => undefined);
    }
  ...
  it("refreshes the execution-target catalog from the open picker", async () => {
    harness.switchExecutionTarget.mockResolvedValue("openai-gpt-5");
  ...
  it("opens the Runtime Available Models read-only view from the route picker without selecting", async () => {
    const createSession = vi.fn(async () => ({
      run: async function* () {}, dispose: vi.fn(), modelCatalog: MODEL_CATALOG,
      switchExecutionTarget: harness.switchExecutionTarget,
    }));
    ...
    try {
      ...
      expect(harness.routePicker?.rows.some((row) => row.content.includes("openai/gpt-5.4"))).toBe(true);
      emitKey("\r", "return"); await flushUi();
      expect(harness.switchExecutionTarget).not.toHaveBeenCalled();
      expect(harness.state?.executionTargetPickerOpen).toBe(true);
  ...
  it("shows a route-catalog loader and blocks selection while refresh is in flight", async () => {
    ...
    const refreshModelCatalog = vi.fn(() => new Promise<void>((resolve) => {
  ...
  it("sends a selected eligible account override through the execution target", async () => {
    ...
    try {
      ...
      expect(harness.switchExecutionTarget).toHaveBeenCalledWith("openai-gpt-5", "work");
    } finally {
      harness.renderer?.destroy();
      void startPromise.catch(() => undefined);
    }
  ...
  it("does not allow selecting an unavailable configured route", async () => {
    harness.switchExecutionTarget.mockResolvedValue("codex-oauth");
  ...
  it("uses the auth-completion route catalog without requesting a second refresh", async () => {
    const authenticatedCatalog = {
      ...
      models: [
        ...
        MODEL_CATALOG.models[1]!,
        {
          ...MODEL_CATALOG.models[2]!,
          availability: "available" as const,
          targets: MODEL_CATALOG.models[2]!.targets.map((target) => ({
            ...target,
            availability: "available" as const,
            reasonCodes: [],
            repairActions: [],
    ...
    let modelCatalog: ModelCatalog = MODEL_CATALOG;
    const refreshModelCatalog = vi.fn(async () => undefined);
    ...
    try {
      ...
      emitKey("\r", "return");
      await flushUi();
      expect(harness.switchExecutionTarget).toHaveBeenCalledWith("codex-oauth");
    } finally {
      harness.renderer?.destroy();
      void startPromise.catch(() => undefined);
    }

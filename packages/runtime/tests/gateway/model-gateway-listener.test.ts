...
import { closeGatewayResources, startCodexOAuthModelGatewayListener, startGateway } from "../../src/gateway/gateway-server.js";
...
describe("startCodexOAuthModelGatewayListener", () => {
  ...
  it("binds only to loopback and closes the listener idempotently", async () => {
    ...
    const stop = vi.fn();
    let bound: Parameters<NonNullable<Parameters<typeof startCodexOAuthModelGatewayListener>[0]["listen"]>>[0] | undefined;
    const runtime = await startCodexOAuthModelGatewayListener({
      config,
  ...
  it("closes durable state even when listener shutdown throws", async () => {
    ...
    const first = await startCodexOAuthModelGatewayListener({ ...options, listen: () => ({ stop: () => { throw new Error("listener stop failed"); } }) });
    expect(() => first.close()).toThrow("listener stop failed");
    const replacementStop = vi.fn();
    const replacement = await startCodexOAuthModelGatewayListener({ ...options, listen: () => ({ stop: replacementStop }) });
    replacement.close();
  });

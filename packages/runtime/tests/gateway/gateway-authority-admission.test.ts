...
import { defineExecutionTargetCatalog, createExecutionAccountPolicyId, createExecutionAccountRef, type ProviderAdapter } from "@kilnai/core/agents";
...
import { FixedTargetGatewayAuthorityAdmission } from "../../src/gateway/gateway-authority-admission.js";
...
async function fixture(overrides: { readonly persistBundle?: (bundle: unknown) => void | Promise<void>; readonly duplicateTarget?: boolean } = {}) {
  ...
  const targets = overrides.duplicateTarget ? [...catalog.targets, { ...catalog.targets[0]!, id: "route-duplicate" }] : catalog.targets;
  const admission = new FixedTargetGatewayAuthorityAdmission({
    appName: "app-1", targetId: "route-1", snapshot: { catalog: { ...catalog, targets }, configurationRevision: { revisionSetId: "gateway-r1", revisions: { global: "global-r1" } } }, sessionRegistry,
...
describe("FixedTargetGatewayAuthorityAdmission", () => {
  it("persists one complete bundle and keeps productive dispatch inside the account fence", async () => {
    ...
    const dispatch = vi.fn(async (commit) => {
      ...
      expect(commit.bundle.turn.execution).toMatchObject({ status: "routed", target: { targetId: "route-1" } });
      ...
      expect(commit.perCallConfig.admittedExecutionTarget).toBeUndefined();
  ...
  it("rejects ambiguous provider/model targets at composition time", async () => {
    await expect(fixture({ duplicateTarget: true })).rejects.toThrow(/exactly one canonical target/iu);
  });
});

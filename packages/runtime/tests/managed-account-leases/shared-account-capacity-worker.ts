...
try {
  ...
  writeFileSync(result, JSON.stringify(authority.acquireAccountCapacity({
    ...
    route,
    candidates: [{ candidate: { account: createAccountRef("configured:account"), route, health: "healthy", leaseCapacity: "available", pressure: 0, reservedForNewWork: false }, capacityIdentity: "stable-account", credentialRevisionId: "a".repeat(64), usageEvidence: { health: "healthy", freshness: "missing" }, capacity: { maxConcurrency: 1, reservedAffinitySlots: 0 } }],
  })));
} finally {
  authority.close();
}

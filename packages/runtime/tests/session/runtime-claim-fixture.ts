...
import {
  ...
  type RuntimeToolActionClaimsContext,
...
function fixtureModelIdentity(provider: ProviderAdapter, model: string | undefined): {
...
export function createFixtureAdmission(input: {
  ...
  const allowedToolPermissions = toolPermissions.filter((permission) => permission.authority.allowed || permission.authority.requiresApproval);
  ...
  const admittedAuthority = allowedToolPermissions.length > 0
    ? allowedToolPermissions.some((permission) => permission.authority.level >= 4)
      ? "destructive" as const
      : allowedToolPermissions.some((permission) => permission.authority.level >= 3) ? "audited" as const : "read_only" as const
    : "fail_closed" as const;
  const effectCeiling = allowedToolPermissions.length > 0
    ? allowedToolPermissions.reduce((effect, permission) => ({
        operation: effect.operation === "mutate" || permission.effectEnvelope.operation === "mutate" ? "mutate" : "observe",
        boundaries: [...new Set([...effect.boundaries, ...permission.effectEnvelope.boundaries])],
        reversibility: effect.reversibility === "irreversible" || permission.effectEnvelope.reversibility === "irreversible" ? "irreversible" : "reversible",
        dataEgress: effect.dataEgress === "sensitive-data" || permission.effectEnvelope.dataEgress === "sensitive-data" ? "sensitive-data" : effect.dataEgress === "project-data" || permission.effectEnvelope.dataEgress === "project-data" ? "project-data" : effect.dataEgress === "metadata" || permission.effectEnvelope.dataEgress === "metadata" ? "metadata" : "none",
        identityUse: effect.identityUse === "privileged" || permission.effectEnvelope.identityUse === "privileged" ? "privileged" : effect.identityUse === "authenticated" || permission.effectEnvelope.identityUse === "authenticated" ? "authenticated" : "none",
        consequences: [...new Set([...effect.consequences, ...permission.effectEnvelope.consequences])],
        idempotency: effect.idempotency === "non-idempotent" || permission.effectEnvelope.idempotency === "non-idempotent" ? "non-idempotent" : effect.idempotency === "conditionally-idempotent" || permission.effectEnvelope.idempotency === "conditionally-idempotent" ? "conditionally-idempotent" : "idempotent",
      } satisfies ActionEffectEnvelope), allowedToolPermissions[0]!.effectEnvelope)
    : {
  ...
  const revision = {
    revisionSetId: "runtime-fixture",
    revisions: { fixture: "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" as `sha256:${string}` },
  } as const;
  ...
  return defineEffectiveAuthorityAdmissionBundle({
    ...
    session: {
      skillCatalog: { catalogId: "runtime-fixture", revision: "sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc", skillIds: [] },
      authorityCeiling: { maximumAuthority: admittedAuthority === "destructive" ? "destructive" : admittedAuthority === "audited" ? "audited" : "read_only", reason: "Fixture admission ceiling", subjectId: input.session.id },
    },
    turn: {
      authority: {
...
export function createFixtureClaimConfig(input: {
  ...
  const modelRound: RuntimeModelRoundDispatchContext = {
    admission,
    intentFingerprint: runtimeModelRoundEffectIdentity({ fixture: "runtime-model-round", sessionId: input.session.id, turnId }),
...
export function createFixtureToolPermission(
  ...
  authority: AuthorityDescriptor = { level: 1, allowed: true, requiresApproval: false, reason: "Fixture admission explicitly allows this tool." },
): FixtureToolPermission {
  const hasUnknownDimension = effectEnvelope.reversibility === "unknown"

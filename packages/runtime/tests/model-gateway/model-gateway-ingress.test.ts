  ...
  virtualModels: [
    {
      ...
      accountIds: ["primary"],
      capabilities: ["text"],
      affinity: { continuity: "prefer", scope: "session" },
    },
    {
      id: "not-allowed",
      displayName: "Other",
...
describe("createModelGatewayIngress", () => {
  ...
  it("composes Responses and Anthropic over one authority while keeping protocol ACL and namespaces distinct", async () => {
    ...
    const shared: ModelGatewayConfig = {
      ...
      surfaces: {
        ...config.surfaces,
        anthropicMessages: { maxBodyBytes: 2048, maxConcurrentRequests: 1 },
      },
      principals: [
        config.principals[0]!,
        {
    ...
    const handle = await createModelGatewayIngress({
      ...
      env: {
        ...
        ANTHROPIC_TOKEN: "a".repeat(32),
      },
    });
    try {
      const responsesPrincipal =
        (await handle.openAIResponses!.authenticateBearer("b".repeat(32)))!;
      const anthropicPrincipal = (await handle.anthropicMessages!.authenticate(
        "a".repeat(32),
      ))!;
      expect(handle.openAIResponses!.invocationPorts).toBe(
        handle.anthropicMessages!.invocationPorts,
      );
      await expect(
        handle.openAIResponses!.resolveVirtualModel({
          principal: responsesPrincipal,
          requestedModel: "claude-codex",
        }),
      ).resolves.toBeUndefined();
      await expect(
        handle.anthropicMessages!.resolveVirtualModel({
          principal: anthropicPrincipal,
          requestedModel: "codex",
        }),
      ).resolves.toBeUndefined();
      await expect(
        handle.anthropicMessages!.listVirtualModels({
          principal: anthropicPrincipal,
        }),
      ).resolves.toEqual([{ id: "claude-codex", displayName: "Claude Codex" }]);
      const responsesCorrelation =
        await handle.openAIResponses!.namespaceCorrelation({
          principal: responsesPrincipal,
          observed: {
            sessionId: "same-session",
            rawBodyDigest: "1".repeat(64),
          },
        });
      const anthropicCorrelation =
        await handle.anthropicMessages!.namespaceCorrelation({
          principal: anthropicPrincipal,
          observed: {
            sessionId: "same-session",
            rawBodyDigest: "1".repeat(64),
      ...
      expect(anthropicCorrelation.sessionId).not.toBe(
        responsesCorrelation.sessionId,
      );
      expect(anthropicCorrelation.turnId).not.toBe(responsesCorrelation.turnId);
      expect(otherAgentCorrelation.sessionId).toBe(
        anthropicCorrelation.sessionId,
      );
      expect(otherAgentCorrelation.turnId).not.toBe(
        anthropicCorrelation.turnId,
      );
    } finally {
      handle.close();
    }
    await expect(
      createModelGatewayIngress({
        ...
        env: {
          REPLAY_SECRET: "r".repeat(32),
          BEARER_TOKEN: "s".repeat(32),
          ANTHROPIC_TOKEN: "s".repeat(32),
        },
      }),
  ...
  it("fails closed when one ingress declares a duplicate trusted principal identity", async () => {
    ...
    await expect(
      ...
      "trusted principal identities must be unique within each ingress",
    );
  });
  ...
  it("uses explicit virtual-model bindings for multiple Codex credentials and fresh usage health", async () => {
    ...
    await pool.linkCredential({
      id: "credential-plus",
      tokenFile: token("account-plus"),
    });
    await pool.linkCredential({
      id: "credential-free",
      tokenFile: token("account-free"),
    });
    vi.stubGlobal(
      "fetch",
    ...
    const multi: ModelGatewayConfig = {
      ...
      virtualModels: [
        { ...config.virtualModels[0]!, accountIds: ["plus", "free"] },
      ],
    ...
    try {
      const principal = (await handle.openAIResponses!.authenticateBearer(
        "b".repeat(32),
      ))!;
      ...
      const catalog =
        await handle.openAIResponses!.invocationPorts.candidateCatalog.list({
          identity: { ...principal, sessionId: "session", turnId: "turn" },
      ...
      expect(
        catalog.candidates.filter(
          (entry) => entry.candidate.health === "healthy",
        ),
      ).toHaveLength(1);
      expect(
        catalog.candidates.find((entry) => entry.candidate.health === "healthy")
          ?.candidate.account,
      ).toContain("configured:free:");
    } finally {
      handle.close();
    }
  });
  ...
  it("dispatches a conforming Messages request through the shared Codex authority exactly once", async () => {
    ...
    await pool.linkCredential({
      id: "credential-a",
      tokenFile: token("provider-account"),
    });
    const anthropicConfig: ModelGatewayConfig = {
      ...config,
      surfaces: {
        anthropicMessages: { maxBodyBytes: 4096, maxConcurrentRequests: 1 },
      },
      principals: [
        {
          tokenEnv: "ANTHROPIC_TOKEN",
    ...
    const handle = await createModelGatewayIngress({
      ...
      databasePath: ":memory:",
      credentialRootDir: credentialRoot,
      env: { REPLAY_SECRET: "r".repeat(32), ANTHROPIC_TOKEN: "a".repeat(32) },
      fetch: providerFetch as typeof fetch,
    });
    try {
      const app = createAnthropicMessagesRoutes(handle.anthropicMessages!);
      const response = await app.request(
        new Request("http://127.0.0.1/v1/messages?beta=true", {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: "user", content: "Return PROBE_OK" }],
          }),
        }),
      );
  ...
  it("composes configured principals, models, and exact credential identities", async () => {
    ...
    try {
      ...
      await expect(
        handle.openAIResponses!.authenticateBearer("wrong".repeat(8)),
      ).resolves.toBeUndefined();
      const resolved = await handle.openAIResponses!.resolveVirtualModel({
        principal: principal!,
        requestedModel: "codex",
      ...
      expect(first?.candidate.account).toMatch(
        /^configured:primary:[a-f0-9]{64}:[a-f0-9]{64}$/,
      );
      expect(String(first?.candidate.account)).not.toContain(
        "provider-account",
      );

      await pool.linkCredential({
        id: "credential-a",
        tokenFile: token("provider-account", "rotated"),
      });
      const {
        candidates: [rotated],
      } = await handle.openAIResponses!.invocationPorts.candidateCatalog.list({
      ...
      expect(rotated?.candidate.account).not.toBe(first?.candidate.account);
      expect(rotated?.capacityIdentity).toBe(first?.capacityIdentity);
      expect(rotated?.credentialRevisionId).not.toBe(
        first?.credentialRevisionId,
      );
      await expect(
        handle.openAIResponses!.invocationPorts.dispatcherResolver.resolve({
          identity,
          route: resolved!.route,
          account: first!.candidate.account,
          leaseId: "authority-held",
          capacityIdentity: first!.capacityIdentity,
          credentialRevisionId: first!.credentialRevisionId,
        }),
      ).rejects.toThrow("credential revision changed");

      const namespaced = await handle.openAIResponses!.namespaceCorrelation({
        principal: principal!,
        observed: {
      ...
      const collidingPrincipal =
        await handle.openAIResponses!.authenticateBearer("c".repeat(32));
      const separatelyNamespaced =
        await handle.openAIResponses!.namespaceCorrelation({
          principal: collidingPrincipal!,
          observed: {
            sessionId: "external-session",
            turnId: "external-turn",
            rawBodyDigest: "a".repeat(64),
      ...
      const retry = await handle.openAIResponses!.namespaceCorrelation({
        principal: principal!,
        observed: {
          sessionId: "external-session",
          rawBodyDigest: "1".repeat(64),
      ...
      const sameRetry = await handle.openAIResponses!.namespaceCorrelation({
        principal: principal!,
        observed: {
          sessionId: "external-session",
          rawBodyDigest: "1".repeat(64),
      ...
      const nextBody = await handle.openAIResponses!.namespaceCorrelation({
        principal: principal!,
        observed: {
          sessionId: "external-session",
          rawBodyDigest: "2".repeat(64),
        },
      });
      expect(sameRetry.turnId).toBe(retry.turnId);
      expect(nextBody.turnId).not.toBe(retry.turnId);
      const threadOnly = await handle.openAIResponses!.namespaceCorrelation({
        principal: principal!,
        observed: { threadId: "stable-thread", rawBodyDigest: "1".repeat(64) },
      });
      const threadNextBody = await handle.openAIResponses!.namespaceCorrelation(
        {
          principal: principal!,
          observed: {
  ...
  it("exposes multiple exact OpenCode accounts and dispatches only the leased account", async () => {
      ...
      virtualModels: [
        {
          ...
          providerId: "opencode-go",
          providerModelId: "k3",
          accountIds: ["go-primary", "go-secondary"],
          capabilities: ["text"],
          affinity: {
            continuity: "prefer",
            scope: "session",
    ...
    try {
      ...
      expect(
        new Set(
          catalog.candidates.map((candidate) => candidate.candidate.account),
        ).size,
      ).toBe(2);
      expect(
        new Set(
          catalog.candidates.map((candidate) => candidate.capacityIdentity),
        ),
      ).toEqual(
        new Set([
          "configured:opencode-go:go-primary",
          "configured:opencode-go:go-secondary",
        ]),
      );
  ...
  it.each([403, 429, 503])(
    "feeds provider status %i into credential health without retrying",
    async (status) => {
      ...
      if (status === 429) {
        ...
        vi.spyOn(
          LocalModelGatewayStore.prototype,
          "coolRoute",
        ).mockImplementation(function (...args) {
          feedbackOrder.push("route-circuit");
          return coolRoute.apply(this, args);
        });
        vi.spyOn(
          CodexOAuthCredentialPoolService.prototype,
          "recordProviderOutcome",
        ).mockImplementation(async () => {
          feedbackOrder.push("credential-health");
      ...
      try {
        ...
        const {
          ...
          { identity, route: resolved.route, authority, budget },
        );
        const dispatcher =
          await handle.openAIResponses!.invocationPorts.dispatcherResolver.resolve(
            {
              identity,
              route: resolved.route,
              account: candidate!.candidate.account,
              leaseId: "authority-held",
              capacityIdentity: candidate!.capacityIdentity,
              credentialRevisionId: candidate!.credentialRevisionId,
            },
          );
        await expect(
          dispatcher.dispatchOneRound({
            account: candidate!.candidate.account,
            route: resolved.route,
            sessionId: "session",
            turn: {
        ...
        if (status === 429)
          expect(feedbackOrder).toEqual(["route-circuit", "credential-health"]);
        const afterFailure =
          await handle.openAIResponses!.invocationPorts.candidateCatalog.list({
            identity,
            route: resolved.route,
        ...
        expect(afterFailure.candidates).toEqual([]);
      } finally {
        handle.close();
      }
    },
  ...
  it("does not mask committed provider success when health persistence fails", async () => {
    ...
    try {
      ...
      ).resolves.toMatchObject({ parts: [{ type: "text", text: "ok" }] });
    } finally {
      handle.close();
    }
  });

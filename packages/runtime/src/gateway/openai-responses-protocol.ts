...
type OutputKind = "message" | "function_call" | "custom_tool_call";
...
export function createResponsesStreamState(response: { responseId: string; model: string }) {
  ...
  return {
    ...
    outputItemDone: (input: { itemId: string; outputIndex: number; item: RecordValue }) => {
      ...
      const kind = completedItem.type; if (kind !== "message" && kind !== "function_call" && kind !== "custom_tool_call") fail("unsupported completed output item type");
      const expectedStage = kind === "message" ? "content_done" : kind === "function_call" ? "arguments_done" : "input_done";
      requireActive(kind, input.itemId, input.outputIndex, [expectedStage]);
      if (completedItem.id !== input.itemId || completedItem.status !== "completed") fail("completed output item identity or status is invalid");
    ...
    completed: (usage: { input_tokens: number; output_tokens: number; total_tokens: number }) => { requirePhase("in_progress"); if (active) fail("active output item must be done before terminal event"); if (!Number.isSafeInteger(usage.input_tokens) || !Number.isSafeInteger(usage.output_tokens) || usage.input_tokens < 0 || usage.output_tokens < 0 || usage.total_tokens !== usage.input_tokens + usage.output_tokens) fail("usage.total_tokens must equal input_tokens + output_tokens"); phase = "terminal"; return event("response.completed", { response: responseObject("completed", { output: structuredClone(completedItems), usage: { ...usage } }) }); },

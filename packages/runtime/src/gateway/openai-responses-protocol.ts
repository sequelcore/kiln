...
export function createResponsesStreamState(response: { responseId: string; model: string }) {
  ...
  return {
    ...
    completed: (usage: { input_tokens: number; output_tokens: number; total_tokens: number; input_tokens_details?: { cached_tokens: number; cache_write_tokens: number } }) => { requirePhase("in_progress"); if (active) fail("active output item must be done before terminal event"); if (!Number.isSafeInteger(usage.input_tokens) || !Number.isSafeInteger(usage.output_tokens) || usage.input_tokens < 0 || usage.output_tokens < 0 || usage.total_tokens !== usage.input_tokens + usage.output_tokens) fail("usage.total_tokens must equal input_tokens + output_tokens"); const details = usage.input_tokens_details; if (details && (!Number.isSafeInteger(details.cached_tokens) || !Number.isSafeInteger(details.cache_write_tokens) || details.cached_tokens < 0 || details.cache_write_tokens < 0 || details.cached_tokens + details.cache_write_tokens > usage.input_tokens)) fail("usage.input_tokens_details must be non-negative subsets of input_tokens"); phase = "terminal"; return event("response.completed", { response: responseObject("completed", { output: structuredClone(completedItems), usage: structuredClone(usage) }) }); },

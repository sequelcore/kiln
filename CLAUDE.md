...
| `commands/run.ts` | `kiln run` command: `permissionPolicy?: KilnPermissionPolicy` on `RunFlags`; registry-driven session selection with circuit breaker fallback. Direct API providers bypass MCP requirements when explicitly selected. Real `toolCount` + `turnDepth` tracking. Persists `.kiln/sessions/{id}/meta.json` + `transcript.jsonl` per session (fail-open). Passes transcript to `SkillGenerator.maybeGenerate()` for two-phase capture. Prints capture hint in cli-wrapper mode without API key. |

...
import { evaluateLemmaScriptQualificationPolicy } from "./qualification-policy.js";
...
describe("LemmaScript qualification policy", () => {
  ...
  it("admits pure helper composition with typed local bindings", () => {
    const helper = { ...TYPED_INFO.functions[0], name: "helper" };
    const composed = {
      ...TYPED_INFO.functions[0],
      name: "composed",
      bodyKinds: ["return", "call", "let", "user", "var"],
    };
    const result = evaluateLemmaScriptQualificationPolicy({
      ...BASE_INPUT,
      typedInfo: { ...TYPED_INFO, functions: [helper, composed] },
      requiredFunctionNames: ["composed"],
    });
